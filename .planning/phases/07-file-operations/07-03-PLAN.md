...
<task type="auto">
  <name>Task 3: Integrate file reading with chat command</name>
  <files>
    crates/cli/src/commands/chat.rs
    crates/core/src/config/types.rs
  </files>
...
2. Update crates/cli/src/commands/chat.rs to inject file context.

   Location: In the `run_chat()` function (or equivalent), BEFORE the message is sent to
   the AI provider but AFTER user input is received. Look for where `CompletionRequest`
   is constructed.
   ...
   b. Read detected files and build context:
   ```rust
   ...
   c. Prepend file context to the user message BEFORE constructing CompletionRequest:
   ```rust
   ...
   ```
   Use `augmented_message` instead of `message` when building the request.

   d. Add tracing for debugging:
   ```rust
...
3. Tests:
  ...
  <done>
    Chat command detects file references and injects their content into AI context
  </done>
</task>
